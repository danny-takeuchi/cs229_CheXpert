...
outGT1, outPRED1 = CheXpertTrainer.test(model, dataLoaderTest, nnClassCount, "model_ones_3epoch_densenet.tar", class_names)
outGT3, outPRED3 = CheXpertTrainer.test(model, dataLoaderTest, nnClassCount, "model_zeroes_1epoch_densenet.pth.tar", class_names)
...
for i in range(nnClassCount):
    ...
    print(i)
    print(metrics.classification_report(outGT1.cpu()[:,i], outPRED1.cpu()[:,i]))
    roc_auc = metrics.auc(fpr, tpr)
    f = plt.subplot(2, 7, i+1)
    fpr2, tpr2, threshold2 = metrics.roc_curve(outGT3.cpu()[:,i], outPRED3.cpu()[:,i])
    print(i)
    print(metrics.classification_report(outGT3.cpu()[:,i], outPRED3.cpu()[:,i]))
