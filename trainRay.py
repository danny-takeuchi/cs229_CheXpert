...
class CheXpertTrainer():
    ...
    def test(self, model, dataLoaderTest, nnClassCount, checkpoint, class_names):   
        ...
        model.eval()
        with torch.no_grad():
            for i, (input, target, patient, study) in enumerate(dataLoaderTest):
                target = target.cuda()
    ...
    def getWeights(self, model, dataLoaderTest, nnClassCount, checkpoint, class_names):   
        ...
        with torch.no_grad():
            for i, (input, target, patient, study) in enumerate(dataLoaderTest):
                ...
                patientsDict[dictKey].append((outPRED, outGT))
                print(patient, study)
        print("EXAMPLE HERE")
        print(dictKey)
        print(len(patientsDict))
        print(patientsDict[('patient05266', 'study1')])
        print(len(patientsDict[dictKey]))
        aurocIndividual = CheXpertTrainer.computeAUROC(self, outGT, outPRED, nnClassCount)
        aurocMean = np.array(aurocIndividual).mean()
